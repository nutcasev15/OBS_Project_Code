...
ADC_PARALLEL_CHANNELS = 4; % Define Number of Concurrent ADC Channels
...
%% Plot ADC Channel 1 - Pin A3 Data
figure(1, 'name', 'ADC Channel 1 - Pin A3 - Voltage Trace');
plot(ADC_CH_time(1, :), ADC_CH_val(1, :));
grid on;
grid minor on;
title('ADC Channel 1 - Voltage Trace');
axis('tight');
ylabel('Voltage (V)');
xlabel('Time (usec)');
set(gca, 'fontsize', 18);

%% Plot ADC Channel 2 - Pin A4 Data
figure(2, 'name', 'ADC Channel 2 - Pin A4 - Voltage Trace');
plot(ADC_CH_time(2, :), ADC_CH_val(2, :));
grid on;
grid minor on;
title('ADC Channel 2 - Voltage Trace');
axis('tight');
ylabel('Voltage (V)');
xlabel('Time (usec)');
set(gca, 'fontsize', 18);

%% Plot ADC Channel 3 - Pin A5 Data
figure(3, 'name', 'ADC Channel 3 - Pin A5 - Voltage Trace');
plot(ADC_CH_time(3, :), ADC_CH_val(3, :));
grid on;
grid minor on;
title('ADC Channel 3 - Voltage Trace');
axis('tight');
ylabel('Voltage (V)');
xlabel('Time (usec)');
set(gca, 'fontsize', 18);

%% Plot ADC Channel 4 - Pin A6 Data
figure(4, 'name', 'ADC Channel 4 - Pin A6 - Voltage Trace');
plot(ADC_CH_time(4, :), ADC_CH_val(4, :));
grid on;
grid minor on;
title('ADC Channel 4 - Voltage Trace');
axis('tight');
ylabel('Voltage (V)');
xlabel('Time (usec)');
set(gca, 'fontsize', 18);
